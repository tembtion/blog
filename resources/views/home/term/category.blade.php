...
@section('title', $category->term->name)

@section('content')
    <div class="title">
        <h2>
            <i aria-hidden="true" class="glyphicon glyphicon-list"></i>
            分类目录归档：{{ $category->term->name }}
        </h2>
    </div>
    <div id="content">
    @if (count($posts) > 0)
        ...
        <div id='pagenav'><a href='{{ URL::route('homeCategoryAjaxIndex', ['page' => 2, 'term_id' => $category->term->term_id]) }}'></a></div>
    @else
