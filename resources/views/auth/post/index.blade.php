            ...
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    <div role="group" id="exampleTableEventsToolbar" class="btn-group hidden-xs">
                        <a class="btn btn-outline btn-default J_menuItem" href="{{ URL::route('authPostCreate') }}">
                    ...
                    <div class="tabs-container m-t">
                        <ul class="nav nav-tabs">
                            <li @if (!isset($filter['post_status'])) class="active" @endif>
                                <a aria-expanded="false" href="{{ URL::route('authPostIndex') }}"> 全部</a>
                            </li>
                            @foreach (config('const.POST_STATUS') as $value)
                                @if ($value['edit'])
                                <li
                                    @if (isset($filter['post_status']) && $filter['post_status'] == $value['value'])
                                        class="active"
                                    @endif
                                >
                                    <a href="{{ URL::route('authPostIndex', ['post_status' => $value['value']]) }}">{{ $value['name'] }}</a>
                                </li>
                                @endif
                            @endforeach
                        </ul>
                    ...
                    @if (count($data) > 0)
                        <table class="footable table table-stripped footable-loaded" id="editable">
                            ...
                            @foreach ($data as $value)
                                <tr class="gradeX">
                                    <th><input type="checkbox" class="btSelect" name="ids[]" value="{{ $value->post_id }}"></th>
                                    <td>
                                        <a target="_blank" href="{{ URL::route('homePostIndex', ['post_id' => $value->post_id]) }}">{{ $value->post_title }}</a> - {{ $post_status_map[$value->post_status] or '' }}
                                    </td>
                                    <td>{{ $value->user->name }}</td>
                                    <td>
                                        @foreach($value->termTaxonomy()->where('taxonomy', 'category')->get() as $tag)
                                            <span>{{ $tag->term->name }}</span>
                                        @endforeach
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <a href="{{ URL::route('authPostCreate', ['post_id' => $value->post_id]) }}">编辑</a><br>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
                            <tfoot>
                            <tr>
                                <td colspan="8"><span class="pull-right">{!! $data->appends($filter)->render() !!}</span></td>
                            </tr>
                            </tfoot>
                        </table>
                    @else
...
$(function(){
    ...
    var delete_url = "{{ URL::route('authPostDelete') }}";
